...
+ Jenkins
  ...
  написанный на Java @jenkins-docs. Он являлся одним из первых инструментов в
  своей области, благодаря чему получил широкое распространение и огромное
  сообщество.
...
+ GitHub Actions
  ...
  Инструмент для автоматизации, встроенный в платформу GitHub @github-docs.
...
+ База данных и файловое хранилище
  ...
  закрытия разработки open-source версии#footnote[URL:
    https://github.com/minio/minio/commit/27742d469462e1561c776f88ca7a1f26816d69e2],
  что значит технология не будет развиваться дальше, было выбрано S3-совместимое
...
+ Конфигурация приложения и парсинг
  ...
  переменных окружения с одновременной валидацией типов @pydantic. Для парсинга
...
+ Реализация клиентских интерфейсов
  ...
  современного подхода с использованием декораторов @click, что делает код более
  чистым и читаемым.
...
Применен декларативный синтаксис SQLAlchemy 2.0 с аннотациями типов:
использование Mapped[] для явного указания типизированных столбцов,
автоматическое сопоставление Python-классов с таблицами БД через
declarative_base().

Программа определяет три сущности с отношениями:
+ Repository (Репозиторий) Содержит уникальный URL и имя репозитория. Имеет
  отношение "один-ко-многим" с Build.
+ Build (Сборка) Основная операционная сущность CI/CD процесса. Содержит
  информацию о коммите (хеш, сообщение, автор, ветка). Отслеживает жизненный цикл
  сборки через временные метки. Хранит ссылки на артефакты и логи сборки.
+ User (Пользователь) Представляет пользователей системы. Содержит идентификатор
  чата для интеграции с мессенджерами.

Реализована детализированная модель состояний сборки, а именно четыре состояния,
покрывающие полный жизненный цикл. Использование Enum обеспечивает безопасность
и валидацию.

Три временных метки в Build (created_at - время создания задачи, started_at -
начало выполнения, finished_at - завершение сборки) позволяют анализировать
производительность системы и рассчитывать метрики.

Каскадное удаление (cascade="all, delete-orphan") обеспечивает автоматическое
удаление зависимых сборок. Схема соответствует третьей нормальной форме:
- Отдельные сущности для репозиториев, сборок и пользователей
- Минимальная избыточность данных
- Четкие отношения между таблицами
...
Файл представляет собой интеграционный компонент системы CI/CD, реализующий
Telegram-интерфейс для взаимодействия с пользователями. Файл выполняет роль
презентационного слоя, обеспечивая:
- Управление пользователями через мессенджер
- Мониторинг статусов сборок
- Распространение уведомлений
- Доставку артефактов

Для создания был использован aiogram 3 - асинхронный фреймворк для Telegram Bot
API, библиотека asyncio - для асинхронного выполнения задач.

В файле прописаны функциональные модули:

+ Регистрация пользователей (cmd_start) отвечает за первичную инициализация
  пользователя в системе. Использует username или user_id как идентификатор,
  сохраняет chat_id для последующих уведомлений, автоматическое создает запись в
  БД.
+ История сборок (cmd_history) отвечает за получение пользователем последних
  сборок репозитория.
+ Получение артефактов (cmd_status) отвечает за получение пользователем артефактов
  сборки в виде 7zip-архива. Команда использует временные файлы
  (tempfile.NamedTemporaryFile).
+ Система уведомлений (send_build_notification) асинхронно рассылает пользователем
  уведомления о статусе сборки, после её окончания.

Структура проекта показана на @image1[Рисунке].

#figure(image("img/image.png"), caption: [Структура проекта]) <image1>

Диаграмма взаимодействия компонентов показана на @image[Рисунке].

#figure(
  image("img/Диаграмма взаимодействия компонентов.drawio.png"),
...
образ, созданный с помощью многоэтапной сборки @docker-build-docs. Для
обеспечения универсальности образ поддерживает две ключевые архитектуры: amd64 и
arm64, что позволяет разворачивать систему на различном серверном оборудовании.
...
В ходе выполнения данной курсовой работы была успешно решена задача по
проектированию и разработке системы непрерывной интеграции "Swompi-Runner".
Разработанная система способна отслеживать изменения в Git-репозиториях,
выполнять пользовательские сценарии в изолированных Docker-контейнерах на основе
конфигурационного YAML-файла, сохранять результаты сборки в S3 хранилище и
уведомлять пользователей о результатах через Telegram.

В процессе работы все задачи, поставленные во введении, были полностью
выполнены:
+ Был проведен анализ предметной области и существующих аналогов, на основе
  которого была спроектирована модульная архитектура приложения.
+ Был выбран и подробно обоснован стек технологий, оптимально подходящий для
  решения поставленных задач.
+ Была разработана и реализована структура базы данных для хранения метаданных о
  репозиториях и сборках.
+ Были полностью реализованы как серверные компоненты, отвечающие за прием
  веб-хуков и выполнение сборок (модуль-исполнитель Executor), так и клиентские
  интерфейсы в виде Telegram-бота и утилиты командной строки.
+ Было разработано и сконфигурировано полноценное окружение для развертывания с
  помощью Docker Compose.
+ Система была успешно развернута на удаленном VPS-сервере.

Основная цель работы, заключавшаяся в приобретении и закреплении
профессиональных навыков, была полностью достигнута. В процессе разработки были
получены глубокие практические знания в области DevOps-практик, контейнеризации
и проектирования многокомпонентных систем.

Среди сложностей, возникших в ходе работы, стоит отметить задачу обеспечения
кросс-платформенной сборки Docker-образов для архитектур AMD64 и ARM64. Также
был получен опыт при настройке взаимодействия с S3 хранилищем Garage. Разработка
скриптов для автоматической инициализации сервисов внутри Docker Compose
позволила глубже понять принципы "Infrastructure as Code".

Таким образом, данная курсовая работа позволила создать работающий инструмент и
получить опыт решения инженерных задач, возникающих при разработке и
развертывании современного программного обеспечения.

#gost.struct-heading("СПИСОК ИСПОЛЬЗОВАННЫХ ИСТОЧНИКОВ")

#bibliography("references.bib", style: "gost-numeric.csl", title: none)

#gost.struct-heading("ПРИЛОЖЕНИЕ А – РЕПОЗИТОРИЙ С КОДОМ")

Исходный код доступен в репозитории: https://github.com/Orlovchikk/Swompi-Runner